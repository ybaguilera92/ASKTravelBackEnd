...
@Service
public class MostVotedStrategy extends QuestionGetStrategy {


    @Override
    public Map<String, Object> getList(BaseDTO baseDTO) {
        pageable = PageRequest.of(baseDTO.getPageIndex(), baseDTO.getPageSize());
        List<Question> questions = questionRepository.findAllByAnswersIsNotNullOrderByCreatedDateDesc().stream().filter(question -> question.getReactions().size() >= 2).collect(Collectors.toList());
